...
// ─── Eligible suppliers jinhone na bid ki na ignore kiya → "missed" record ───
const recordMissedBids = async (bulkOrder) => {
  ...
  for (const si of eligible) {
    ...
    if (existing) continue; // bid ya ignored already hai
...
const runWinnerSelect = async (settings) => {
  ...
  try {
    ...
    for (const bulkOrder of activeBulkOrders) {
      ...
      if (!winningBid) {
        if (bulkOrder.retryCount >= 3) {
          ...
          if (retryIds.length > 0) {
            ...
            // NOTE: retry pe bidding band nahi hui → missed record NAHI banate
          } else {
            await BulkOrder.findByIdAndUpdate(bulkOrder._id, { status: "cancelled" });
            await recordMissedBids(bulkOrder);  // final cancel → missed
          }
        }
        continue;
      }
      ...
      // baaki sab jinhone bid ki thi (price wale) → lost
