...
const { uploadToFirebase, bucket } = require("../config/uploadToFirebase");
const {
...
// ─── Helper: Delete old file from Firebase by URL ────────────────────────────
const deleteFromFirebaseByUrl = async (url) => {
  if (!url || !bucket) return;
  try {
    const filePath = decodeURIComponent(url.split("/o/")[1]?.split("?")[0]);
    if (filePath) await bucket.file(filePath).delete();
  } catch (err) {
    console.warn("deleteFromFirebaseByUrl: could not delete old file:", err.message);
    // Error ignore — upload will still continue
  }
};
...
exports.uploadContract = async (req, res) => {
  try {
    ...
    // ✅ Delete old contract from Firebase first
    await deleteFromFirebaseByUrl(branch.contractPdf);
...
exports.uploadPdc = async (req, res) => {
  try {
    ...
    if (req.file) {
      // ✅ Delete old PDC image from Firebase first
      await deleteFromFirebaseByUrl(branch.pdcImage);
