...
exports.approveReceipt = async (req, res) => {
  try {
    ...
    for (const inv of buyerInvoices) {
      ...
      // ─── Supplier ka share release karo (3% platform rakh ke) ───
      const supplierAmount = Math.round(
        (inv.grandTotal - inv.commissionAmount - inv.deliveryAmount) * 100
      ) / 100;
      ...
      if (supplierInv) {
        await Invoice.findByIdAndUpdate(supplierInv._id, {
          ...
          amountDue:             0,
          amountPaid:            supplierAmount,
          grandTotal:            supplierAmount,
        });
        totalReleased += supplierAmount;
        suppliersReleased++;
      }
    }
    ...
    res.json({
      success: true,
      message: `Payment approved ✅ ${suppliersReleased} supplier invoices released.`,
      data: {
        suppliersReleased,
        totalReleased: Math.round(totalReleased * 100) / 100,
      },
