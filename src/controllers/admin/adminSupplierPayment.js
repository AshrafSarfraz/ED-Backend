...
exports.getPaymentDays = async (req, res) => {
  try {
    ...
    entries.forEach(e => {
      ...
      d.totalAmount += signed; // sab entries (order_earning, reversal, penalty) shamil — sahi "net earned"
      if (e.category === "return_penalty") d.totalDeducted += e.amount; // sirf display ke liye alag bhi track
...
exports.getDayBulkOrders = async (req, res) => {
  try {
    ...
    // ─── Buyer-side payment status — "did money actually come IN" (from buyer, or
    //     via rider-recovery), SEPARATE from "did we release it OUT to supplier" ───
    const buyerOrderIds = [...new Set(invoices.filter(i => i.buyerOrderId).map(i => i.buyerOrderId.toString()))];
    const [buyerInvoices, returnOrders] = await Promise.all([
      Invoice.find({ buyerOrderId: { $in: buyerOrderIds }, invoiceType: "buyer" })
        .select("buyerOrderId paymentStatus").lean(),
      ReturnOrder.find({ buyerOrderId: { $in: buyerOrderIds } })
        .select("buyerOrderId status").lean(),
    ]);
    const buyerInvoiceMap = {};
    buyerInvoices.forEach(bi => { buyerInvoiceMap[bi.buyerOrderId.toString()] = bi; });
    const returnOrderMap = {};
    returnOrders.forEach(ro => { returnOrderMap[ro.buyerOrderId.toString()] = ro; });

    const getBuyerPaymentInfo = (buyerOrderId) => {
      if (!buyerOrderId) return { buyerPaymentStatus: "unpaid", buyerPaymentLabel: "Pending" };
      const bi = buyerInvoiceMap[buyerOrderId.toString()];
      if (!bi) return { buyerPaymentStatus: "unpaid", buyerPaymentLabel: "Pending" };

      if (bi.paymentStatus === "paid") {
        return { buyerPaymentStatus: "paid", buyerPaymentLabel: "Paid" };
      }
      if (bi.paymentStatus === "cancelled") {
        // Buyer didn't pay — but money is still coming in, just from a different source
        const ro = returnOrderMap[buyerOrderId.toString()];
        if (ro?.status === "resolved_rider_guilty") {
          return { buyerPaymentStatus: "paid_rider_recovery", buyerPaymentLabel: "Paid (Rider Recovery)" };
        }
        return { buyerPaymentStatus: "cancelled", buyerPaymentLabel: "Returned — No Payment" };
      }
      return { buyerPaymentStatus: "unpaid", buyerPaymentLabel: "Pending" };
    };
    ...
    Object.entries(perInvoice).forEach(([invId, agg]) => {
      ...
      const buyerPaymentInfo = getBuyerPaymentInfo(inv.buyerOrderId);
      ...
      b.buyerOrders.push({
        ...
        // "release" — humne supplier ko diya ya nahi (jo pehle sirf "status" tha)
        releaseStatus: agg.allSettled ? "released" : "pending",
        // "buyer payment" — buyer/rider ki taraf se paisa aaya ya nahi (naya, alag)
        buyerPaymentStatus: buyerPaymentInfo.buyerPaymentStatus,
        buyerPaymentLabel:  buyerPaymentInfo.buyerPaymentLabel,
  ...
  try {
    const summary = await LedgerEntry.aggregate([
      { $match: { entityType: "supplier" } },
      {
        $group: {
          _id: "$entityId",
          // "Total Earned" ab NET hai (credit - debit, sab categories: order_earning,
          // order_earning_reversal, return_penalty) — Daily Records screen ke fix jaisa
          // hi, taake dono jagah consistent number dikhein
          totalEarned:   { $sum: { $cond: [{ $eq: ["$direction", "credit"] }, "$amount", { $multiply: ["$amount", -1] }] } },
