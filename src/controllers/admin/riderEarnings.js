...
exports.getEarningDetail = async (req, res) => {
  try {
    ...
    // Har entry se item detail nikaal lo — ek hi jagah, earnings aur debts dono use karte hain
    const withItem = (e) => ({
      ...
      orderAmount:  e.invoiceId?.totalAmount ?? null, // raw order value (fees se pehle)
    });
...
// ═══════════════════════════════════════════════════════
//  ADMIN — Total recovered from riders (settled debts where rider owed us more
//  than they earned) — separate from normal "payout" (money WE gave out).
//  GET /api/admin/rider-earnings/recovered
// ═══════════════════════════════════════════════════════
exports.getRecoveredFromRiders = async (req, res) => {
  try {
    // flowDirection="recovery" for new records; fall back to amount<0 for any
    // older Payout docs created before this field existed — doesn't break old data.
    const payouts = await Payout.find({
      entityType: "rider",
      $or: [{ flowDirection: "recovery" }, { flowDirection: null, amount: { $lt: 0 } }],
    }).lean();

    const totalRecovered = Math.round(
      payouts.reduce((s, p) => s + Math.abs(p.amount || 0), 0) * 100
    ) / 100;

    res.json({ success: true, totalRecovered, count: payouts.length, data: payouts });
  } catch (err) {
    console.error("getRecoveredFromRiders error:", err);
    res.status(500).json({ success: false, message: "Server error" });
  }
};
