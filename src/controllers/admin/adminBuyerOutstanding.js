...
// 📁 controllers/admin/adminBuyerOutstanding.js
// ═══════════════════════════════════════════════════════
//  BUYER OUTSTANDING — exact mirror of the supplier flow, 3 levels
//    Level 1  /days                            → day-wise (bill-wise)
//    Level 2  /days/:date/buyers               → that day's buyers
//    Level 3  /days/:date/buyers/:branchId     → one buyer's invoices + bill
//    Export   /export
//
//  The supplier side is ledger-driven (money we OWE).
//  The buyer side is Invoice-driven (money we're OWED) — via paymentStatus/amountDue.
//
//  The old /api/admin/buyer-payments is untouched and still running.
//  This newer router is mounted on a separate path: /api/admin/buyer-outstanding
// ═══════════════════════════════════════════════════════
const Invoice     = require("../../models/invoice");
const Branch      = require("../../models/Branch");
const Company     = require("../../models/createCompany");
...
// Cancelled = return resolved as supplier-at-fault. The buyer no longer owes this.
const isLive = (inv) => inv.paymentStatus !== "cancelled";
...
exports.getBuyerDayDetail = async (req, res) => {
  try {
    ...
    // The branch record may be missing (deleted buyer, or orphaned invoice data),
    // but the invoices still belong to this buyerBranchId — so show the bill anyway.
    // Level 2 already degrades this way; Level 3 was the only place that hard-failed
    // with a 404, which made the whole day's billing unreachable for that buyer.
    const branch = (await Branch.findById(branchId)
      .select("managerName companyName companyId email phone address").lean()) || {};
    ...
    const buyerInfo = {
      ...
      // true when the branch document no longer exists — the frontend can flag it
      branchMissing: !branch.managerName,
      address:      branch.address || null,
    };
...
// ═══════════════════════════════════════════════════════
//  EXPORT — GET /api/admin/buyer-outstanding/export
//  ?onlyDue=true → only what is still outstanding
// ═══════════════════════════════════════════════════════
exports.exportBuyerOutstanding = async (req, res) => {
  try {
    const onlyDue  = String(req.query.onlyDue || "") === "true";
